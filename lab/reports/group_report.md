...
- **Peer review (slide Phần E — 3 câu hỏi):**
  1. *Nếu pipeline không có freshness check, agent sẽ gặp vấn đề gì?* → Agent đọc chunk stale mà không biết, confident trả lời sai.
  2. *Tại sao idempotency quan trọng với vector store?* → Không có prune → mỗi rerun phình collection, vector cũ lạc hậu ảnh hưởng retrieval.
  3. *`hits_forbidden` phát hiện gì mà `contains_expected` không phát hiện được?* → `contains_expected` chỉ check top-1; `hits_forbidden` quét toàn bộ top-k context — phát hiện chunk sai trong context dù top-1 đúng.
